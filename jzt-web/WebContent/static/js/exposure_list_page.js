...
function AjaxPage(CurPage, PageSize) { 
	...
	$.ajax( {  
        ...
        success : function(data) {  
        	if (data == null) {  
                 $('#tbody').html("<tr><td><h2>未找到相关曝光！</h2></td><td></td><td></td><td></td><td></td></tr>");
                 $('#paging').empty();                
             }  
        	 else {
        		 if (eval(data["expList"]).length != 0) {
        			 var str = '';  
        			 for (var i = 0; i < eval(data["expList"]).length; i++) {  
                      	var product=eval(data["expList"]);
                      	product=product[i];
                      	str += '<tr>'+
							      	'<td class="table-style"><a href="'+programPath+'/exposure/detail?eid='+product["eid"]+'">'+product["title"]+'</a></td>'+
							      	'<td class="table-style"><font class="small">'+product["username"]+'<br>'+
								      		''+getFormatDateByLong(product["createdDate"], "yyyy-MM-dd")+''+
							      		'</font>'+
							      	'</td>'+
							      	'<td class="table-style"><font class="blue">'+product["commentCnt"]+'</font>/'+product["clickCnt"]+'</td>'+
							      	'<td class="table-style"><img class="img-thumb" src="'+programPath+'/static/image/thumb_up.png" alt=""/><font class="red"> '+product["agreeCnt"]+'</font></td>'+
							      	'<td class="table-style"><font class="small">'+product["lastCommenter"]+'<br>'+
							      			''+getFormatDateByLong(product["lastComment"], "yyyy-MM-dd hh:mm")+''+
							      		'</font>'+
							      	'</td>'+
							      '</tr>';
     		 		  }
