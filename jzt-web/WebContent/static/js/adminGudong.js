...
$(function(){
	...
	$("#add_partner").click(function(){
		var html="<tr>"+
			  		"<td id=\"partner_name\"><input name=\"name\" placeholder=\"合作伙伴名称\"/></td>"+
			  		"<td id=\"partner_style\"><select name=\"type\" class=\"form-control select_input\" >"+
					"<option value=\"传统金融机构\">传统金融机构</option>\<option value=\"第三方支付\">第三方支付</option>\<option value=\"风险投资机构\">风险投资机构</option>\<option value=\"其他金融从业公司\">其他金融从业公司</option>\<option value=\"财经信息门户网站\">财经信息门户网站</option>\<option value=\"法律支持\">法律支持</option>\<option value=\"技术支持\">技术支持</option>\<option value=\"研究机构\">研究机构</option>\<option value=\"综合媒体\">综合媒体</option>\<option value=\"相关细分行业公司\">相关细分行业公司</option>\<option value=\"国际组织\">国际组织</option>\<option value=\"其他\">其他</option>\</select></td>"+
			  		"<td>"+
			  			...
			  			"<button class=\"btn btn-primary edit_item hide\"  ><span class=\"glyphicon glyphicon-edit\"></span>&nbsp;修改</button>"+
			  			"<button class=\"btn btn-primary edit_save_item hide\"  ><span class=\"glyphicon glyphicon-edit\"></span>&nbsp;保存</button>"+
			  			"<button class=\"btn btn-primary add_item\" style=\"margin-left:2px;\"><span class=\"glyphicon glyphicon-edit\"></span>&nbsp;新增</button>"+
			  		"</td>"+
			  	"</tr>";
	...
	$("#partner_table").on("click",".edit_item",function(){
		...
		$(parent).find("#partner_style").html("<select name=\"type\" class=\"form-control select_input\" >"+
					"<option value=\"传统金融机构\">传统金融机构</option>\<option value=\"第三方支付\">第三方支付</option>\<option value=\"风险投资机构\">风险投资机构</option>\<option value=\"其他金融从业公司\">其他金融从业公司</option>\<option value=\"财经信息门户网站\">财经信息门户网站</option>\<option value=\"法律支持\">法律支持</option>\<option value=\"技术支持\">技术支持</option>\<option value=\"研究机构\">研究机构</option>\<option value=\"综合媒体\">综合媒体</option>\<option value=\"相关细分行业公司\">相关细分行业公司</option>\<option value=\"国际组织\">国际组织</option>\<option value=\"其他\">其他</option>\</select>");
			...
			$.ajax({ 
				...
				success: function (data) {
					...
					else if (state==1)
					{
						...
						//$(parent).attr("value").value(data.partner.id);
						//alert(data.partner.id);
						$(parent).attr("value",data.partner.id);
					}
					else if (state==2)
					{
			...
			$.ajax({ 
				...
				success: function (data) {
					...
					else if (state==1)
					{
						...
						$('.add_item').hide();
						$(parent).attr("value",data.association.id);
...
function getLogResult(url){
	window.open (url,'log','height=600,width=1200,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no');
}